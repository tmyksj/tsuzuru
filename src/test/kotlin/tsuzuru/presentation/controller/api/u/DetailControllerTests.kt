...
class DetailControllerTests {
    ...
    fun get_responds_200() {
        controllerSupport.perform {
            MockMvcRequestBuilders.get("/api/u/${entitySupport.userEntity(scope = UserEntity.Scope.Public).name}")
    ...
    fun get_responds_404_when_user_does_not_exist() {
        controllerSupport.perform {
            MockMvcRequestBuilders.get("/api/u/${UUID.randomUUID()}")
    ...
    fun get_responds_404_when_user_sets_scope_to_private() {
        controllerSupport.perform {
            MockMvcRequestBuilders.get("/api/u/${entitySupport.userEntity(scope = UserEntity.Scope.Private).name}")
        }.andExpect(MockMvcResultMatchers.status().isNotFound)
    }
